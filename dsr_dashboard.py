...
HISTORICAL_DATA_FILE = (
    'historical_data/historical_sales.csv.gz'
)
...
class DSRDashboard:
    ...
    def load_data(self):
        ...
        try:
            ...
            if not date_columns:
                ...
                raise ValueError(
                    "Date / Sales_Date column not found!"
                )
            ...
            logger.info(
                ...
                f"Report Target Date: {self.today}"
            )
    ...
    def get_kpi_cards_data(
        ...
        lmtd_end = (
            previous_month_date
            .replace(day=target_day.day)
            .date()
        )


        lmtd_start = (
            lmtd_end.replace(day=1)
        )
        ...
        # LY MTD
        #
        # 1st previous year month
        # -> SAME DAY NUMBER as target day
        #
        # Example:
        # Target = 15-Sep-2026
        # LY MTD = 01-Sep-2025 -> 15-Sep-2025
        # ----------------------------------------------------
        ...
        mom = self.calculate_growth(
            mtd['net_sales'],
            lmtd['net_sales']
        )


        yoy = self.calculate_growth(
            mtd['net_sales'],
            lytd['net_sales']
        )
    ...
    def get_summary_table(
        ...
        # ----------------------------------------------------
        # Yesterday / Target Day
        # ----------------------------------------------------

        yest_m = self.get_metrics_dict(
        ...
        ly_mtd_m = self.get_metrics_dict(
    ...
    def get_bucket_analysis(
        ...
        ftd_total = (
            ftd_data['Net Sales'].sum()
        )


        mtd_total = (
            mtd_data['Net Sales'].sum()
        )
    ...
    def get_day_level_performance(
        ...
        for d in dates:
            ...
            sub = mtd_data[
                mtd_data['Date'].dt.date
                == d
            ]


            m_dict = (
                self.get_metrics_dict(sub)
            )


            matrix['Net Sales'].append(
                m_dict['net_sales']
            )


            matrix['Discount'].append(
                m_dict['discount']
            )


            matrix['Orders'].append(
                m_dict['orders']
            )
...
1. Overall Sales Summary (COCO + FOFO)

</h3>

{self.render_table_html(
...
9. Current Month Day Level Performance (COCO)

</h3>

{self.render_table_html(
    self.get_day_level_performance()
)}
...
"""
    ...
    def send_dashboard_email(
        ...
        try:
            ...
            logger.info(
                "✅ DSR Dashboard email sent "
                "successfully to all recipients!"
            )
...
def send_telegram_summary(self):
    ...
    try:
        ...
        # ====================================================
        # IMPORTANT:
        # These methods must already exist in your actual
        # working Telegram version:
        #
        # get_ftd_metrics()
        # get_mtd_metrics()
        # get_brand_data()
        ...
        # get_session_data()
        #
        # They were not present in the script pasted here.
        # ====================================================
        ...
        ftd_msg = [
            ...
            *[
            ...
            f"{ftd.get('max_dis_channel', 'N/A')} "
            f"| *Region:* "
            f"{ftd.get('max_dis_region', 'N/A')}"
        ...
        mtd_msg = [
            ...
            *[
            ...
            f"\n💡 *Highest discount channel:* "
            f"{mtd.get('max_dis_channel', 'N/A')} "
            f"| *Region:* "
            f"{mtd.get('max_dis_region', 'N/A')}"
        ...
        logger.error(
            f"❌ Failed to send Telegram summary: "
            f"{e}"
        )
...
# ============================================================
# ATTACH TELEGRAM FUNCTION TO CLASS
# ============================================================

DSRDashboard.send_telegram_summary = (
...
def main():
    ...
    # 3. GOOGLE SHEET
    #
    # Keep your update_google_sheet() function here
    # when the Google Sheet module is added.
    # --------------------------------------------------------
